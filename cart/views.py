...
class AddToCartView(CartMixin,View):
    ...
    def post(self,request,slug):
        ...
        if size_id:
            ...
            product_size = product_size.filter(stock__gt=0).first()
            if not product_size:
                return JsonResponse({
                    'error': 'No sizes availeble'
                },status = 400)
        ...
        if existing_item:
            ...
            if total_quantity > product_size.stock:
                return JsonResponse({
                    'error':f"Cannot add {quantity} items.Only {product_size.stock - existing_item.quantity} more avaible"
                },status = 400)
    ...
    def post(self,request,item_id):
        ...
        if quantity == 0:
            ...
            if quantity > cart_item.product_size.stock:
                return JsonResponse({
                    'error':f'Only {cart_item.product_size} items avaialable'
                })
